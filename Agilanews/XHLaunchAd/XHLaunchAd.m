...
- (instancetype)init
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:UIApplicationWillEnterForegroundNotification object:nil queue:nil usingBlock:^(NSNotification * _Nonnull note) {
            ...
            NSNumber *show_time = DEF_PERSISTENT_GET_OBJECT(SS_SPLASH_SHOW_TIME);
            NSNumber *reboot_time = DEF_PERSISTENT_GET_OBJECT(SS_SPLASH_REBOOT_TIME);
            long long time = [[NSDate date] timeIntervalSince1970] - show_time.longLongValue;
            if (on.integerValue && time > reboot_time.longLongValue) {
                // 开屏广告
