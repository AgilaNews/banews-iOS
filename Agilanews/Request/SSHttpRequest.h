 ...
 *  @param failure 请求失败后的回调（请将请求失败后想做的事情写到这个block中）
 */
- (void)post:(NSString *)url params:(id)params contentType:(ContentType)contentType serverType:(NetServerType)serverType success:(void (^)(id responseObj))success failure:(void (^)(NSError *error))failure isShowHUD:(BOOL)showHUD;
